...
import { SUPPORTED_CHAIN_ID, Web3sdksSDK } from "@web3sdks/sdk";
...
const polygonSDK = Web3sdksSDK.fromPrivateKey(privateKey, "polygon");
...
async function main() {
  ...
  if (latest && latest.metadataUri) {
    ...
    for (const [chainId, implementation] of Object.entries(implementations)) {
      ...
      const chainSDK = Web3sdksSDK.fromPrivateKey(privateKey, chainName);
  ...
  console.log("Release page:", `https://web3sdks.com/${releaser}/${releasedContractName}`);
}
